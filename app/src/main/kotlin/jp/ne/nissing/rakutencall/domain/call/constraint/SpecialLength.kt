...
class SpecialLength @Inject constructor(
        private val settingDataSource: SettingDataSource,
        private val contactsDataSource: ContactsDataSource,
        executionThreads: ExecutionThreads)
    : IoUseCase<SpecialLength.Request, SpecialLength.Response, Throwable>(executionThreads) {
    override fun execute(requestValue: Request): Observable<Response> {
        return settingDataSource.getSpecialNumIgnoreEnable().flatMap {
            if (it)
                contactsDataSource.getSpecialNumberLength().map {
                    if (requestValue.phoneNumber.number.length <= it) {
